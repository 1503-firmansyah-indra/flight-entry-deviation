...
def main(sector, oversampling_factor, include_own_pt, normalize_video_occ, buffer, grid_lon, grid_lat, threshold,
         comb, train_vec_model, train_occ_model, ensemble, n_estimators, max_depths, learning_rate):
    # load labels
    ...
    X_occ_flat = X_comp_occ.reshape((len(X_comp_occ), -1))

    # Combining Occupancy Matrix
    X = np.array(list(zip(X_vec, X_occ_flat)), dtype=object)
    ...
    for each_seed in seed_list:
        ...
        occ_model = None
        if comb:
            comb_model = XGBClassifier(n_estimators=n_estimators, max_depth=max_depths, learning_rate=learning_rate,
                                       objective='binary:logistic')
        if train_vec_model or ensemble:
            vec_model = XGBClassifier(n_estimators=n_estimators, max_depth=max_depths, learning_rate=learning_rate,
                                      objective='binary:logistic')
        if train_occ_model or ensemble:
            occ_model = XGBClassifier(n_estimators=n_estimators, max_depth=max_depths, learning_rate=learning_rate,
                                      objective='binary:logistic')
    ...
    df_results.to_csv(f"../results/results_xgb_sector{sector}.csv", index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-md", "--max-depth", type=int, default=6, help="max_depth of the XGB classifiers")
    parser.add_argument("-lr", "--learning-rate", type=float, default=1.0, help="learning rate of XGB classifiers")
    parser.add_argument("-ne", "--n-estimators", type=int, default=120, help="number of estimators in XGB classifiers")
